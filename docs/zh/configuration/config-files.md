...
当主模型看不见图片（其 [`[models]`](#models) 条目缺少 `image_in` 能力）时，请求中的图片通常会被降级为路径占位文本。`[cap_route]` 节提供了另一种做法：`image_route` 指向一个视觉模型，由它把每张图片转成文字描述，描述文本在原位置替换图片，主模型仍能基于图片内容作答。

| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `image_route` | `string` | — | 用于描述图片的视觉模型：一个具备 `image_in` 能力的 [`[models]`](#models) 条目别名。引用语义与 `default_model` 相同——别名可以悬空；别名不存在或指向无视觉能力的模型时视同未配置（记一条警告日志），启动不会失败 |
...
- 主模型的能力不含 `image_in`；
- 请求中包含图片（上传的图片与直传 `image_url` 一视同仁）；
- 请求是一个轮次——压缩及其他内部操作请求不会路由；
- `image_route` 已配置且解析到有效的视觉模型。

同一张图片对同一个视觉模型只描述一次：描述会被持久化缓存，跨轮次、跨 Agent 复用。旁路调用失败时，图片退回既有的降级行为（路径占位文本），主请求照常发出。视觉模型消耗的 token 计入会话用量，归属配置的别名。

::: warning 注意
配置 `image_route` 即授权将图像内容与问题摘要（截断后）发送到该视觉模型对应的端点——包括第三方或公有云视觉服务。运行时没有任何确认机制，配置文件是唯一的控制点。
:::
